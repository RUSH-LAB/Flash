...
#if defined GPU_HASHING || defined GPU_TB
...
#endif
#if defined GPU_HASHING && defined CPU_TB
...
#endif
#if defined CPU_HASHING && defined GPU_TB
...
void LSHReservoirSampler::ann(int numQueryEntries, float* queries, unsigned int* outputs, int topk) {
	...
	kSelect(tally, outputs, segmentSize, segmentSizePow2, numQueryEntries, topk);
	delete[] tally;
#endif
#if defined DEBUG
	std::cout << "[LSHReservoirSampler::ann] K-selection completed. " << std::endl;
...
#if defined GPU_TB || defined GPU_HASHING
	clReleaseMemObject(allprobsHash_gpuobj);
	clReleaseMemObject(allprobsIdx_gpuobj);
#endif
